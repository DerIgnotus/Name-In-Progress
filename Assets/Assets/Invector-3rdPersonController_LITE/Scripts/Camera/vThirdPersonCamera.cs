...
public class vThirdPersonCamera : MonoBehaviour
{
    ...
    private List<float> distances = new List<float> { 1.5f, 2.5f, 3.5f, 4.5f, 5.5f };
    ...
    {
        if (!gameManager.GetCanMove()) return;
        if (Input.mouseScrollDelta.y > 0) DistanceNumCalc(currentDistance - 1);
        else if (Input.mouseScrollDelta.y < 0) DistanceNumCalc(currentDistance + 1);
    }

    void DistanceNumCalc(int newDistNum)
    {
        if (newDistNum < 0) currentDistance = 0;
        else if (newDistNum > 4) currentDistance = 4;
        else
        {
            currentDistance = newDistNum;
            defaultDistance = newDistNum;
        }
        distance = distances[currentDistance];
        defaultDistance = distances[currentDistance];
    }
